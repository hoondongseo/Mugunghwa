...
export function MessageFeed({ selectedRegion }: MessageFeedProps) {
  ...
  const filteredMessages = () => {
    ...
    switch (activeFilter) {
      ...
      case "popular":
        return [...baseMessages].sort((a, b) => (b.likes || 0) - (a.likes || 0));
      case "region":
        return selectedRegion 
          ? baseMessages.filter(msg => msg.region === selectedRegion)
          : baseMessages;
      default:
        return baseMessages;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-12">
        <h3 className="text-3xl font-bold text-gray-900 mb-4">전국 응원 메시지</h3>
        <p className="text-lg text-gray-600">대한민국 곳곳에서 온 따뜻한 응원의 목소리</p>
      </div>
            ...
            <Card key={message.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900">{message.region}</div>
                ...
                <p className="text-gray-800 mb-4 line-clamp-3">{message.content}</p>
