...
export class MemStorage implements IStorage {
	...
	async getMessagesByRegion(region: string, limit = 50): Promise<Message[]> {
		return Array.from(this.messages.values())
			.filter(
				(message) => message.region === region && message.isApproved
			)
			.sort(
				(a, b) =>
					new Date(b.createdAt!).getTime() -
	...
	async createMessage(insertMessage: InsertMessage): Promise<Message> {
		...
		if (region) {
			await this.updateRegionMessageCount(
				region.name,
				(region.messageCount || 0) + 1
			);
		}
	...
	async updateRegionMessageCount(
		regionName: string,
		count: number
	): Promise<Region | undefined> {
	...
	async getTotalRegionsCount(): Promise<number> {
		return Array.from(this.regions.values()).filter(
			(region) => (region.messageCount || 0) > 0
		).length;
	}
	...
	> {
		...
		return regions.map((region) => ({
			...
			percentage:
				totalMessages > 0
					? Math.round(
							((region.messageCount || 0) / totalMessages) * 100
					  )
					: 0,
		}));
	}
